-- Aspiral: Master Walkthrough (Experimental Framework)
...
CREATE TABLE sensor_data (
    ...
    sensor_id int,
    reading double precision -- Aspiral: stats, ohlc
); 
...
SELECT aspiral_refresh('sensor_data_1m');

-- Stage 3: Transparent Query Acceleration
\echo ''
\echo '--- [STAGE 3] Transparent Query Acceleration ---'
-- Let's use the new aspiral_explain to see the plan before executing
SELECT aspiral_explain('SELECT sum(reading) FROM sensor_data WHERE t >= ''2025-01-01 00:00:00Z''::timestamptz AND t < ''2025-01-01 01:00:00Z''::timestamptz');
...
CREATE TABLE weather_raw (
    t timestamptz NOT NULL, 
    temp double precision -- Aspiral: sum
); 
...
SELECT aspiral_refresh('weather_raw_1m');

\echo '-> Joining two Aspiral tables on time (Diagnostic Explain):'
SELECT aspiral_explain('SELECT sum(s.reading + w.temp) FROM sensor_data s JOIN weather_raw w ON s.t = w.t WHERE s.t >= ''2025-01-01 00:00:00Z''::timestamptz AND s.t < ''2025-01-01 01:00:00Z''::timestamptz');

-- Stage 5: masterpiece API and Storage
\echo ''
\echo '--- [STAGE 5] Masterpiece API & Storage ---'
\echo '-> Aspiral Status (sensor_data):'
SELECT aspiral_status('sensor_data');

\echo '-> Epoch Conversions:'
SELECT '2025-01-01 12:00:00Z'::timestamptz as human, aspiral_to_epoch('2025-01-01 12:00:00Z') as address;

\echo '-> Packing data into 30x smaller Block XOR format...'
SELECT aspiral_pack_delta_blocks('sensor_data', 12345);

\echo '-> Storage footprint on disk:'
\! ls -lh /tmp/aspiral_main/12345_blocks.bin
