-- Unified Aspiral Benchmark: Comprehensive Feature Validation
...
-- Explain should show a standard Seq Scan without any Aspiral notices or hook modifications
\echo '-> Validating non-aspiral EXPLAIN:'
...
SELECT aspiral_register_view('stress_raw_1m', 'BASE', 60, 'stress_raw', ARRAY[]::text[]);
SELECT aspiral_register_view('stress_raw_1h', 'stress_raw_1m', 3600, 'stress_raw', ARRAY[]::text[]);
...
SELECT aspiral_refresh('stress_raw_1m'); 
...
SELECT aspiral_register_view('iot_raw_1m', 'BASE', 60, 'iot_raw', ARRAY['device_id', 'sensor_type']::text[]);
SELECT aspiral_register_view('iot_raw_1h', 'iot_raw_1m', 3600, 'iot_raw', ARRAY['device_id', 'sensor_type']::text[]);
...
SELECT aspiral_refresh('iot_raw_1m');
...
\echo '-> EXPLAIN: Joining two independent Aspiral hierarchies (Propagation Test)'
EXPLAIN (COSTS OFF)
