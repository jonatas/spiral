-- Aspiral Walkthrough: Multi-Level Hierarchical Query Acceleration
...
\echo '--- ASPIRAL LINEAGE REGISTRY ---'
SELECT view_name, base_column, formula, mat_column FROM aspiral.sources ORDER BY view_name, mat_column;

-- 2. Look at the metadata
-- This tracks the hierarchy and parent-child relationships.
\echo '--- ASPIRAL HIERARCHY ---'
SELECT view_name, parent_view, frame_seconds FROM aspiral.metadata;

-- 3. Transparent Query Acceleration (The Magic)
-- We will query the RAW table, but watch how Aspiral rewrites it.
...
-- Aspiral will detect the "dirty" minute and fall back to the raw table ONLY for that minute,
