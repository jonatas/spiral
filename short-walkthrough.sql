-- Aspiral: Zero-Config Lifecycle Walkthrough
...
-- Aspiral will automatically detect 't' as time and create the view hierarchy.
DROP TABLE IF EXISTS asset_ticks CASCADE;
CREATE TABLE asset_ticks (
    t timestamptz NOT NULL,
    symbol text NOT NULL,
    price double precision, -- Aspiral: ohlc as p, sketch as p_sketch
    vol int                 -- Aspiral: sum as volume
) WITH (
    aspiral.frames = '1m,5m,1h', 
    aspiral.tenant = 'symbol'
);
...
SELECT 
    symbol, 
    round(aspiral_quantile(p_sketch, 0.95)::numeric, 2) as p95,
    round(aspiral_quantile(p_sketch, 0.99)::numeric, 2) as p99
...
-- Verify Aspiral tracked the dirty bucket
SELECT '--- Changelog Check ---' as msg;
SELECT base_view, to_timestamptz(t_start) as bucket_t FROM aspiral.changelog;
