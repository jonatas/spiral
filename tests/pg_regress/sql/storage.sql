...
-- 3. Pack
SELECT spiral_pack_delta_zero('test_storage', 9999);

-- 4. O(1) Read
SELECT spiral_read_main_zero(9999, 0, 0) as t0_d0;
SELECT spiral_read_main_zero(9999, 1, 1) as t1_d1;
SELECT spiral_read_main_zero(9999, 100, 5) as t100_d5;

-- 5. Scan & Reconstruct
SELECT t, tenant_id, value FROM spiral_scan_zero(9999) ORDER BY t, tenant_id;

-- 6. Verify with original
SELECT COUNT(*), AVG(value) FROM spiral_scan_zero(9999);
SELECT COUNT(*), AVG(price) FROM test_storage;

-- 7. Safety Check: Pace mismatch
SET spiral.minimal_pace = 1.0;
-- This should fail validation
SELECT spiral_read_main_zero(9999, 0, 0);

-- 8. Safety Check: OID mismatch
-- Try to read OID 9999 data using OID 8888
SELECT spiral_read_main_zero(8888, 0, 0);
