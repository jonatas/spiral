-- End-to-End Backup/Restore Test for Aspiral Zero-Timestamp Storage
...
SELECT aspiral_pack_delta_zero('production_data', 7777);
\! ls -lh /tmp/aspiral_main/7777_zero.bin

-- 2. SIMULATE BACKUP
\echo '--- [BACKUP PHASE] ---'
-- Standard pg_dump would not see the binary files.
-- We create a backup table using aspiral_scan_zero()
CREATE TABLE backup_reconstructed AS SELECT * FROM aspiral_scan_zero(7777);
...
SET aspiral.kickoff_date = '2026-04-01';
SET aspiral.minimal_pace = 1.0;
SELECT aspiral_pack_delta_zero('restore_source', 7777);
...
SELECT 
    ...
    aspiral_read_main_zero(7777, t, 0) as price_tenant_0,
    aspiral_read_main_zero(7777, t, 1) as price_tenant_1
FROM generate_series(0, 4) t;

-- Compare counts
SELECT COUNT(*) as restored_count FROM aspiral_scan_zero(7777);
