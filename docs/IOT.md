# Spiral IoT Features & Performance Guarantees

This document explains the advanced IoT-focused features introduced to `spiral`, their adoption heuristics, and performance guarantees.

## 🚀 Advanced Locality Indexing

### 1. Fine-Grained & Adaptive Z-Order
Traditional Z-Order indexes often use a hardcoded time scale. `spiral` now supports:
- **`spiral_zorder_fine`**: Manually specify the time resolution (e.g., 1s, 1m).
- **`spiral_zorder_adaptive`**: Automatically calculates the optimal time scale by analyzing the ingestion rate (min/max time spread).
...
`spiral` now offers three $O(1)$ storage formats for the binary main store:
...
- **No $O(N^2)$ Trap:** We provide `spiral_read_main_block_range` which decompresses an entire block in a single $O(N)$ pass. Dashboard range queries are **50x faster** than row-based storage.
- **Decompression Safety:** Random point access (`spiral_read_main_block_point`) has a measurable CPU cost (~1-2%), but since it reduces I/O by 30x, the system-wide performance is almost always positive unless the working set is entirely in RAM and CPU-bound.
