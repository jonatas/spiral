...
- **The Merge Advantage**: Because this is based on moments, we can combine two buckets \(A\) and \(B\) using parallel merge logic without ever needing to see the raw data points that created them. This is the secret to Aspiral's \(O(1)\) rollups.
